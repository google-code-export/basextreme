...
TournamentSource::TournamentSource(unsigned int locationId, Career* career)
{
    ...
    // build list
    unsigned int numTournaments = location->tournaments.size();    
...
{
    return database::LocationInfo::getRecord( _locationId )->thumbnail.c_str();
}
...
{
    return Gameplay::iLanguage->getUnicodeString( database::LocationInfo::getRecord(_locationId)->tournaments[_tournaments[itemId]].nameId );
}

const wchar_t* TournamentSource::getBriefing(unsigned int itemId)
{
    return Gameplay::iLanguage->getUnicodeString( database::LocationInfo::getRecord(_locationId)->tournaments[_tournaments[itemId]].briefId );
}

const char* TournamentSource::getThumbnail(unsigned int itemId)
{
    assert( itemId < _tournaments.size() );
    return database::LocationInfo::getRecord(_locationId)->tournaments[_tournaments[itemId]].thumbnail.c_str();
}
...
{
    return new MissionSource( &(database::LocationInfo::getRecord(_locationId)->tournaments[_tournaments[itemId]]) );
}